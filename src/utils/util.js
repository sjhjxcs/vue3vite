...
export function timeFix() {
  ...
  return hour < 9 ? i18n.t('i18n_util1') : (hour <= 11 ? i18n.t('i18n_util2') : (hour <= 13 ? i18n.t('i18n_util3') : (hour < 20 ? i18n.t('i18n_util4') : i18n.t('i18n_util5'))))
}

export function welcome() {
  const arr = [i18n.t('i18n_util6'), i18n.t('i18n_util7'), i18n.t('i18n_util8'), i18n.t('i18n_util9')]
...
export function generateIndexRouter(data) {
  const indexMenu = { path: '', title: '' }
  const children = generateChildRouters(data, indexMenu)
  ...
  const indexRouter = [
    {
      ...
      meta: { title: indexMenu.title },
      redirect: indexMenu.path,
      children: [
        ...children
      ]
    },
...
function generateChildRouters(data, indexMenu) {
  const routers = []
  for (const item of data) {
    ...
    if (item.component.indexOf('layouts') >= 0) {
      ...
      if (indexMenu && indexMenu.path === '') {
        indexMenu.path = item.path
        indexMenu.title = item.meta.title
      }
    }
    ...
    if (item.children && item.children.length > 0) {
      menu.children = [...generateChildRouters(item.children, indexMenu)]
    }
...
export function jsExpand(options = {}) {
  ...
  window[windowKeyName]['EVENT_' + id] = {
    ...
    // 当js运行出错的时候调用的事件
    error(e) {
      console.group(`${options.errorMessage || i18n.t('i18n_util10')}（${new Date()}）`)
      console.error(e)
      console.groupEnd()
    }
  }
...
export function alwaysResolve(promise) {
  return new Promise((resolve, reject) => {
    ...
    if (p instanceof Promise) {
      ...
      reject(`alwaysResolve: ${i18n.t('i18n_util11')}`)
    }
  })
}
...
/**
 * @param {*} value any
 * @param {*} list [{value: key, text: xxx}]
 * @returns string
 */
export const getTextByValue = (value, list) => {
  const res = {}
  if (list instanceof Array) {
    list.forEach(obj => {
      if (Object.prototype.toString.call(obj) === '[object Object]') {
        res[obj.value] = obj.text
      }
    })
  }
  console.log(res)
...
/**
 * 生成后端需要的参数对象
 * @param queryParam vue中表单对象
 * @param interfaceParam 需要上传给后端的接口对象
 * @param ailas 别名 表单对象与上传给后端的接口对象不对应的参数列表{key为表单对象的key,value为接口对象的key}
 */
export const generateParam = (queryParam, interfaceParam, ailas) => {
  if (isEmpty(queryParam)) {
    return interfaceParam
  }
  if (isEmpty(ailas)) {
    Object.keys(interfaceParam).forEach(key => {
      interfaceParam[key] = queryParam[key] || ''
    })
  } else {
    Object.keys(interfaceParam).forEach(key => {
      if (ailas[key]) {
        interfaceParam[key] = queryParam[ailas[key]]
      } else {
        interfaceParam[key] = queryParam[key] || ''
      }
    })
...
export function dfs(node, target) {
  if (!node) return null // 如果节点为空，返回null

  if (node.path === target) {
    return node // 如果节点值等于目标值，返回该节点
  }

  if (!node.children) { // 如果节点没有子节点，直接返回null
    return null
  }

  for (let i = 0; i < node.children.length; i++) { // 遍历子节点
    const result = dfs(node.children[i], target) // 递归调用dfs函数查找
    if (result) {
      return result // 如果找到了目标节点，返回该节点
    }
  }
...
export function findInTrees(trees, target) {
  for (let i = 0; i < trees.length; i++) { // 遍历数组中的每个树状结构对象
    const result = dfs(trees[i], target) // 调用dfs函数查找目标值
    if (result) {
      return result // 如果找到了目标节点，返回该节点
    }
  }
...
//     if (filter.indexOf(node.dataIndex) != -1) {
//       newTree.push(node);
//       if (node.children) {
//         newTree[newTree.length - 1].children = filterTree(node.children, filter);
//       }
//     } else if (node.children) {
//       const filteredChildren = filterTree(node.children, filter);
//       if (filteredChildren.length > 0) {
//         newTree.push({ ...node, children: filteredChildren });
//       }
//     }
//   }
//   return newTree;
...
/**
 * 根据条件过滤树 栈 非递归版本的实现可能会导致树的顺序发生变化
 * @param {*} tree
 * @param {*} filter
 * @returns
 */
...
//         if (filter.indexOf(child.dataIndex) != -1) {
  ...
  for (const node of tree) {
    if (typeof node === 'object' && node !== null) {
      if (Array.isArray(node)) {
        newTree.push(deepCloneTree(node))
      } else {
        newTree.push(deepCloneWithObserver(node))
      }
    } else {
      newTree.push(node)
    }
...
function deepCloneWithObserver(obj) {
  ...
  Object.getOwnPropertyNames(obj).forEach(prop => {
    Object.defineProperty(clone, prop, Object.getOwnPropertyDescriptor(obj, prop))
  })
 ...
 * @param code
 */
export function mapDictFromCache(code, value) {
  if (isEmpty(code)) {
    return value
  }
  return getTextByValue(value, Vue.ls.get('UI_CACHE_DB_DICT_DATA')[code])
}

export const openPageWithToken = (url, tokenValue) => {
  const encodedTokenValue = encodeURIComponent(tokenValue)
  const urlWithToken = `${url}?token=${encodedTokenValue}`
  const windowFeatures = 'fullscreen=yes, toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes'
  window.open(urlWithToken, '_blank', windowFeatures)
}
